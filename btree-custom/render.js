...
const draw = matrix => {
	matrix.forEach((row, depth, matrix) => {
		...
		row.forEach((node, index, row) => {
			...
			var attachDOM // = svg;
			if (node.parent != null) {
				attachDOM = node.parent.group.append('svg:g').attr('id', node.code)
			} else {
				attachDOM = svg.append('svg:g').attr('id', node.code)
			}
			...
			if (node.parent) {
				...
				var y2 =
					...
					parseInt(
						d3
							.select('[id="' + node.parent.code + '--rect:' + 0 + '"]')
							.attr('y')
					)
		...
		row.forEach((node, index, row) => {
			...
			node.children.forEach((child, childIndex, children) => {
				...
				var circle = d3.select(
					'[id="' + nodeCode + '--circle:' + childIndex + '"]'
				)
				var newCircle = node.group
					.append('svg:circle')
